...
* Description: Kurze Beschreibung, wird rechts neben dem Namen angezeigt.
* Command: Der eigentliche Befehl, welcher beim Bestätigen des Menüeintrags aufgerufen wird.
* Arguments: jede Art von festen oder dynamischen Befehlsoptionen. Der Text zwischen `<` und `>` wird als Platzhalter erkannt und im Menü beim Aufrufen abgefragt.
...
**Hinweis**: Feldnamen und spezielle Platzhalter in Menüdateien sind sprachunabhängig und müssen englische Schlüssel verwenden. Die Oberfläche wird separat über GNU gettext übersetzt.
...
* `ATMIRAL_LANG`: Optionale Gettext-Sprachvorgabe, z. B. `de`. Leer lassen, um die Systemsprache zu verwenden.
...
Ausgaben werden direkt an ihrer Verwendungsstelle mit `_ "..."` übersetzt. Nach dem Ändern oder Hinzufügen von Meldungen werden die Kataloge so aktualisiert:
